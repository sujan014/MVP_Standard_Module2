    ...
    public class ProfileService : IProfileService
    {
        ...
        public async Task<IEnumerable<TalentSnapshotViewModel>> GetTalentSnapshotList(string employerOrJobId, bool forJob, int position, int increment)
        {
            ...
            {   // Rubbish code
                //try
                //{
                //    IQueryable<User> allUsers = _userRepository.GetQueryable();
                //    //var builder = Builders<User>.Filter;
                //    //FilterDefinition<User> filter = builder.Where(x => x.JobSeekingStatus.GetType() != typeof(string));

                //    //var allUsers = await _userRepository.FindAsync(user => typeof(user.JobSeekingStatus) != typeof(string));
                //    //var allUsers = _userRepository.GetQueryable().Where(x => x.JobSeekingStatus.GetType() == typeof(JobSeekingStatus));                
                //    var filteredUsers = allUsers.Skip(position).Take(increment);
                //    var lists = filteredUsers.Select(x => ViewModelFromTalent(x)).ToList();
                //    return lists;
                //}
                //catch (Exception ex)
                //{
        ...
        protected TalentSnapshotViewModel ViewModelFromTalent(User user)
        {
            ...
            return new TalentSnapshotViewModel
            {
                ...
                PhotoId = user.ProfilePhotoUrl != null ? user.ProfilePhotoUrl : "",
                VideoUrl = user.VideoName != null ? user.VideoName : "",
                CVUrl = user.CvName != null ? user.CvName : "",
                Summary = user.Summary != null ? user.Summary : "",
                CurrentEmployment = "Meta",//user.JobSeekingStatus.Status != null ? user.JobSeekingStatus.Status : "", // Do this later
                Visa = user.VisaStatus != null ? user.VisaStatus : "",
                Level = "Software Developer",     // do this later
                Skills = SkillsFromUserSkills(user.Skills),
            };
        }
        protected List<string> SkillsFromUserSkills(List<UserSkill> skills)
        {
            List<string> result = new List<string>();
            foreach (UserSkill skillName in skills) 
            {
                result.Add(skillName.Skill);
            }
            return result;
        }
